...
function SupportPage() {
  ...
  const onToggleAccordion = (ind: any) => {
    setToggleAccordion((prev) => (prev === ind ? -1 : ind));
  };
      ...
      <div className={styles.pg_container}>
        ...
        <article className={styles.pg_content}>
          <div className={styles.hgroup}>
            <h2 className={styles.hgroup_title}>{currentTitle}</h2>
            <p>You can find all answers you need right here</p>
          </div>

          {items.map((i: any, k: number) => (
            <dl className={styles.card} key={k}>
              <dt
                className={styles.card_header}
                onClick={() => onToggleAccordion(k)}
              >
